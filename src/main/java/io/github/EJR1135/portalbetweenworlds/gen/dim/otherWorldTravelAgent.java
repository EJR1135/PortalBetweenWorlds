...
public class otherWorldTravelAgent extends PortalForcer {
    private final Random rand = new Random();

    public otherWorldTravelAgent() {
    }
    ...
    public boolean teleportToValidPortal(World world, Entity entity) {
        ...
        for (int j1 = l - c; j1 <= l + c; ++j1) {
            ...
            for (int j2 = i1 - c; j2 <= i1 + c; ++j2) {
                ...
                for (int k2 = 127; k2 >= 0; --k2) {
                    if (world.getBlockId(j1, k2, j2) == otherWorldBlocks.Portal.id) {
                        while (world.getBlockId(j1, k2 - 1, j2) == otherWorldBlocks.Portal.id) {
                            --k2;
                        }
        ...
        if (d >= 0.0) {
            ...
            d6 = (double) k + 0.5;
            if (world.getBlockId(i - 1, j, k) == otherWorldBlocks.Portal.id) {
                d2 -= 0.5;
            }
            ...
            if (world.getBlockId(i, j, k - 1) == otherWorldBlocks.Portal.id) {
                d6 -= 0.5;
            }
            ...
            entity.setPositionAndAnglesKeepPrevAngles(d2, d4, d6, entity.yaw, 0.0F);
